...
import { useGetReviewsByIdQuery } from "@/services/KinowayService";
...
export const ReviewsInfo: FC<ReviewsInfoProps> = ({ limit }) => {
  ...
  const goodReviews = docs?.filter(
    (rev) => rev?.type === "Позитивный" || !rev.type
  ).length;
  const badReviews = docs?.filter((rev) => rev?.type === "Негативный").length;
  const neutralReviews = docs?.filter(
    (rev) => rev?.type === "Нейтральный"
  ).length;

  const items = [
    { quantity: total, caption: "Всего" },
    {
      quantity: goodReviews,
      percent: 100, //calcPercent(goodReviews, total)
      caption: "Положительные",
      color: "var(--color-green)",
    },
    {
      quantity: badReviews,
      percent: 40, //calcPercent(badReviews, total)
      caption: "Отрицательные",
      color: "var(--color-red)",
    },
    {
      quantity: neutralReviews,
      percent: 20, // calcPercent(neutralReviews, total)
      caption: "Нейтральные",
      color: "#777",
    },
  ];
